...
- TUTOR SOURCES ALL COURSES (June 2026): slide transcripts ingested for courses 224 (doc 51), 225 (doc 52), 226 (doc 53) — tutor now grounded with citations on every published course (verified cited answer on 226). Course 243 was doc 50.
...
- MAINTENANCE (June 2026): GitHub push was rejected (files >100MB: ifpi_lms.db 64MB, out2/out3.webm ~108MB, node_modules webpack cache 132MB — 49k node_modules files were tracked). Fixed: added *.db, backend/uploads/, node_modules/, __pycache__/, *.pyc, frontend/build/ to .gitignore; untracked them; purged all from git history via git-filter-repo. Repo pack shrank 234MiB → 6.35MiB. Working files untouched on disk; app verified healthy (backend+frontend 200). Next "Save to GitHub" should succeed.
- PRODUCTION LOAD REHEARSAL (June 2026): Ran load_test.py against https://learning-hub-968.emergent.host. Results in load_tests.json (labels: production_rehearsal, prod_5users, prod_10users, preview_external). FINDING: the 187rps preview number was localhost (no proxy). Through Cloudflare from a SINGLE source IP, throughput caps ~15-19rps regardless of concurrency (5u=18.9rps, 10u=14.3rps, 30u=8.6rps) — preview via its OWN external proxy also drops to 16rps, proving it is edge per-IP throttling, NOT backend capacity. At realistic per-user concurrency (5u) prod latency is healthy: p50 190-490ms, p95 <650ms, 0 errors. Verdict: production is fit for staff rollout; single-IP rps benchmarks through Cloudflare are not meaningful.
- VERIFIED (June 2026): Course Completion Certificates ALREADY FULLY BUILT (auto-issue in enrollment_service.complete + scorm_xapi, branded PDF w/ QR verify, learner CertificatesPage download/share, admin revoke/bulk/CSV). Verified live on prod: learner has 19 certs, PDF download 200 OK. Note: uat-learner's old certs show course_title=None (courses deleted, test debris) — PDF falls back to "IFPI Course".
- TEST DATA SWEEP (June 2026, user-approved, EXECUTED on Neon prod DB): Backed up ALL 64 tables (94,234 rows) to /app/backend/backups/neon_backup_20260816_053502.tar.gz (gitignored). Then ran scripts/test_data_sweep.py (--dry-run then --execute): deleted 17,311 rows — 388 fake orgs, 1,009 test users (kept 10 real: grant@edgefitness.co.za, admin/learner/qa-admin@ifpi.org, uat-admin/uat-learner@ifpi.org, 2 migration bots, templates@system.local, owner@threesixtyerp.com), 87 faker courses, all UAT-account certs/enrollments/views/notifications/badges. KEPT: org 1 (IFPI Main Academy: courses 1,20,21,22,24,27,40-42,221-223) + org 327 (UAT Sandbox: 224-226, 243 SCORM Module 1), all 52 RAG source_documents, learner@ifpi.org's 4 legit certs. Script uses recursive FK-graph walker (information_schema), reusable for future sweeps.
- CRITICAL DISCOVERY (June 2026): PRODUCTION (learning-hub-968.emergent.host) IS STILL THE PRE-NEON BUILD running the old bundled SQLite file — proven via marker test (Neon name change invisible to prod) + prod data exactly matches ifpi_lms.db (19 uat certs, 1019 users). The user NEVER REPUBLISHED after the Neon migration. Prod DATABASE_URL fallback = sqlite. USER MUST REPUBLISH to switch prod to the cleaned Neon DB. deployment_agent scan: PASS, no blockers. After republish, verify prod /api/certificates for uat-learner returns 0 and catalog shows only real courses.
- PRODUCTION NEON CUTOVER COMPLETE (June 2026): Root cause chain resolved — (1) prod deploy secrets are managed in Manage Publishing -> Secrets ("Edit Custom keys"), NOT synced from workspace .env; (2) prod's DATABASE URL secret was still sqlite:///./ifpi_lms.db; (3) first fix attempt failed because user pasted instruction text into the secret ("...require Then save and rede") -> added field_validator sanitizer in core/config.py (strips quotes/whitespace/trailing text); (4) /api/health now reports {db, build} for instant env diagnosis. User replaced the secret with the Neon URL and republished. VERIFIED LIVE: db=postgresql, build 2026-06-16.2, uat-learner 0 certs, clean catalog, all 4 logins OK, learner's 4 real certs intact, Object Storage media 200, latency p50 43-393ms p95<740ms, 0 errors. Production data is now PERSISTENT across redeploys. Load test history: prod_neon_final in memory/load_tests.json.
- DUPLICATE CLEANUP (June 2026, user-approved): Archived UAT Sandbox duplicates 224/225/226 via POST /api/courses/{id}/archive (reversible via /unarchive). Copied Module 1 (course 243, 99 slides) into IFPI Main Academy as NEW COURSE 294 (scripts/copy_module1_to_org1.py) incl. its RAG transcript doc (new source_document 54, 30 chunks w/ embeddings). Media URLs shared (object storage, not org-scoped). VERIFIED on prod: UAT learner sees only Module 1 (243); main academy learner sees 294+1+221-223; course 294 detail loads 99 slides, media 200, AI tutor answers Module 1 questions with citations in org 1. Source of truth: all real content now in org 1.
- QUALIFICATION PATHWAYS (June 2026, from 4 uploaded curriculum PDFs): Built on LearningPath (added metadata_json col; certificates got learning_path_id col — ALTERed on Neon). New services/pathway_service.py (map + check_and_award_qualifications, hooked into enrollment_service.complete, idempotent), routers/pathways.py (GET /api/pathways/map). Certificates: QUALIFICATION type resolves designation in listing/admin-list/verify/OG/PDF (PDF shows "Qualification · NQF 4 · N Credits" + unit standards footer). Seeded via scripts/seed_pathways.py: 3 DRAFT courses — #295 M2 Facility (9 bites), #296 M3 Exercise Training (48 bites), #297 M2 Group Exercise (25 bites) with per-bite TEXT slides; prereqs 294→295, 294→297, 295→296; tracks #64 Fitness Instructor (NQF4, 28cr, 5 unit standards, designation "Professional Fitness Instructor") & #65 Group Exercise (NQF4, 15cr, "Certified Group Fitness Coach"). Frontend: pages/dashboard/PathwaysPage.tsx at /pathways, nav "Qualifications" (admin+learner). TESTED: map API, award+idempotency (temp user, cleaned up), PDF/verify/admin-list, screenshot OK. NOTE: 3 new courses are DRAFT ("In production" state) until team adds videos and publishes.
- MODULES PUBLISHED + RPL + COMPLIANCE DASHBOARD (June 2026): (1) scripts/publish_modules.py published courses 295/296/297 with text-bite content (draft markers stripped) and ingested each into RAG (docs 55-57) for AI tutor grounding — learner map now shows locked (prereq) instead of coming_soon; (2) RPL: enrollments.via_rpl col (ALTERed on Neon + model), pathway_service.grant_rpl/revoke_rpl (COMPLETED w/o course cert, counts toward prereqs+qualification, audit-logged, triggers qualification check), endpoints POST/DELETE /api/pathways/admin/rpl (ADMIN only, 403 for learners — tested); (3) Compliance: GET /api/pathways/admin/completions (+.csv export) — learner×module matrix per track (states: completed/rpl/in_progress/not_started + qualified flag). FIXED during testing: SELECT DISTINCT over users JSON col fails on PG — removed .distinct() (uq_user_role makes it redundant). Frontend: AdminPathwaysPage at /pathways/admin (track tabs, clickable RPL cells with confirm, CSV download), "Track Compliance" button on PathwaysPage for admins. FULLY TESTED via curl (RPL grant→M2s unlock→matrix shows 'rpl'→revoke restores) + screenshots. NOTE: admin@ifpi.org has forced password change in UI; qa-admin@ifpi.org used for UI testing.
- MODULE QUIZZES + STAFF INVITES (June 2026): (1) Quiz gates — created 4 published exams via scripts/create_module_quizzes.py using the existing AI question generator: exam 13→course 294 (9 Qs), 14→295 (10), 15→296 (10), 16→297 (9), all MCQ, pass 70%, 3 attempts, randomized. ADDED server-side gate in enrollment_service.complete: published course exam without a passed attempt → 412 {exam_id, exam_title} (frontend Iter-49 gate already routes learners to /take/{exam_id} and completes after pass). VERIFIED e2e with temp account: complete blocked 412 → exam pass 100% → complete 200 → Module 2s unlocked. NOTE: RPL grants intentionally bypass the quiz gate (admin action, doesn't call complete). (2) Staff invites ALREADY FULLY BUILT (UsersPage InviteModal + BulkInviteModal, /api/admin/invitations, public accept at /accept-invite/:token) — verified e2e: create → email queued in outbox ("You're invited to IFPI Main Academy") → public lookup → accept creates account + auto-login. Invite emails go to DEV OUTBOX until real SMTP configured. All test users/invites cleaned from DB (incl. outbox_messages + persons FK deps).
- CAMPAIGN LINKS (June 2026): Public prospect acquisition. New CampaignLink model (models/identity.py, table auto-created), routers/campaign_links.py: admin CRUD at /api/admin/campaign-links (name, optional auto-enroll published free course, is_active toggle, signup_count) + public GET/POST /api/join/{slug}(/signup) — creates LEARNER + Person(source=campaign:<name>), auto-enrolls, increments counter, returns LoginResponse w/ cookies. Frontend: public /join/:slug (CampaignSignupPage, branded, shows auto-enroll course), "Campaign Links" tab on UsersPage (components/CampaignLinksPanel.tsx: create w/ course picker, copy URL, signups counter, Active/Paused toggle, delete). VERIFIED e2e via curl (create→lookup→signup→counter=1→dup 409→prospect sees Module 1 in_progress→pause→404) + screenshot. Test prospect+link cleaned up. Question Review & Attempt Analytics confirmed ALREADY BUILT (Exams → Attempts → question-insights panel w/ Edit modal + miss rates + CSV; verified API lists all questions pre-attempts).
- UTM ATTRIBUTION + PROSPECT NURTURING (June 2026): (1) New campaign_signups table (link_id, user_id, utm_source/medium/campaign, nudged_at). /join page forwards utm_* query params into signup; GET /api/admin/campaign-links/{id}/attribution groups signups by source/medium; CampaignLinksPanel has per-link BarChart3 expand showing breakdown. (2) Nurturing: organizations.nurture_enabled/nurture_days/nurture_message cols (ALTERed on Neon), services/nurture_service.run_nurture_pass (finds signups older than N days with no enrollment progress>0, sends outbox email + in-app notification with {name}/{course} placeholders, sets nudged_at — idempotent), daily APScheduler job 08:00 UTC (_nurture_tick in outbox_worker), admin endpoints PUT /api/organization/nurture-settings + POST .../run-now, "Prospect Nurturing" card in Settings page (toggle/days/message/Run now). VERIFIED e2e: 2 UTM signups → attribution grouped correctly → enabled 1-day nurture → backdated → run-now sent 2 nudges (emails in outbox + notifications) → rerun sent 0 (idempotent). Test data cleaned; nurture reset to DISABLED (admin opts in). Nudge emails go to DEV OUTBOX until SMTP configured (org SMTP columns exist on organizations table).
- TREND CHART + SECOND NUDGE (June 2026): (1) attribution endpoint now returns 30-day zero-filled daily `trend`; CampaignLinksPanel renders CSS bar chart (trend-chart-{id}) above UTM table. (2) Second nudge: organizations.nurture_second_enabled/nurture_second_days + campaign_signups.second_nudged_at (ALTERed on Neon). nurture_service refactored with _send_nudge helper; second pass sends "Final reminder" (template nurture_final) N days after first nudge if still no progress. Settings card has "Final reminder" sub-toggle + days input. VERIFIED e2e: trend buckets correct; run sequence 1st nudge → 0 (gap) → backdate → final sent → 0 (idempotent); both emails in outbox. Test data cleaned; nurture reset to DISABLED.
- CAMPAIGN QR CODES (June 2026): GET /api/admin/campaign-links/{id}/qr?base=<origin> returns print-ready 912x912 PNG (qrcode[pil] lib, ERROR_CORRECT_H for poster durability, attachment filename qr-<name>.png; base validated http(s) only — 400 otherwise). QrCode button in CampaignLinksPanel downloads via blob with base=window.location.origin. VERIFIED: valid PNG magic + dims, invalid base rejected 400, test link cleaned. requirements.txt refrozen with qrcode/pillow.
- UAT ACCOUNTS MOVED TO ORG 1 (June 2026): User reported "no learning paths visible" — root cause: uat-admin/uat-learner were in org 327 (UAT Sandbox) while tracks live in org 1 (data is org-scoped, NOT a bug). Per user choice (a): moved both UAT users + persons to organization_id=1. Also swept 62 leftover debris learning paths (TEST_path_*, Career Track A, Untitled) from org 1 admin view. Remaining paths: IFPI Core Path, Fitness Instructor Track, Group Exercise Instructor Track. VERIFIED on prod: both UAT accounts see tracks + pathway map + all courses. Org 327 (archived duplicates + original Module 1 course 243) still exists but has no active users.
