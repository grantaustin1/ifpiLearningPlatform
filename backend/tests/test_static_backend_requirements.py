...
# Mirrors svglib==2.0.2's declared lower bound for reportlab.
SVGLIB_2_0_2_REPORTLAB_SPEC = ">=4.4.3"
# Mirrors xhtml2pdf==0.2.17's declared reportlab compatibility window.
...
def _pinned_version(requirement):
    return next(
        (str(spec)[2:] for spec in requirement.specifier if str(spec).startswith("==")),
        None,
    )
...
def test_google_genai_websockets_pin_compatibility():
    ...
    pinned_version = _pinned_version(websockets_req)
...
def test_reportlab_pin_compatibility():
    ...
    pinned_version = _pinned_version(reportlab_req)
    assert pinned_version is not None, "reportlab must be pinned with == in requirements.txt"
    pinned = Version(pinned_version)

    assert pinned in SpecifierSet(SVGLIB_2_0_2_REPORTLAB_SPEC)
    assert pinned in SpecifierSet(XHTML2PDF_0_2_17_REPORTLAB_SPEC)
