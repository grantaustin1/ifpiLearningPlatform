...
type SettingsTab = 'branding' | 'documents' | 'security' | 'compliance'
...
<<<<<<< HEAD
  ...
  const savePreset = async () => {
    if (!presetForm.name?.trim()) { toast.error('Preset needs a name'); return }
    setSavingPreset(true)
    try {
      if (presetForm.id) await api.put(`/organization/themes/${presetForm.id}`, presetForm)
      else await api.post('/organization/themes', presetForm)
      toast.success(presetForm.id ? 'Preset updated' : 'Preset created')
      setPresetForm(null)
      await loadThemes()
    } catch (e: any) { toast.error(e?.response?.data?.detail || 'Could not save preset') }
    finally { setSavingPreset(false) }
  }

  const deletePreset = async (t: any) => {
    if (!await confirm({
      title: `Delete preset "${t.name}"?`,
      description: 'Academies that already applied it keep their colours — only the preset entry is removed.',
      confirmLabel: 'Delete', variant: 'danger',
    })) return
    try {
      await api.delete(`/organization/themes/${t.id}`)
        ...
        <Section icon={Sparkles} title="Theme presets" help="Hover for Preview / Apply. Preview renders the cert with the preset's colors WITHOUT saving. Apply persists. Create your own presets with New preset.">
          <div className="flex justify-end mb-2">
            <button type="button" onClick={() => setPresetForm({ ...EMPTY_PRESET })}
              ...
              className="text-xs font-semibold text-indigo-600 hover:text-indigo-700 hover:bg-indigo-50 px-2.5 py-1.5 rounded-md inline-flex items-center gap-1.5">
              <Plus className="h-3.5 w-3.5" /> New preset
            </button>
          </div>
          {presetForm && (
            <div className="border border-indigo-200 bg-indigo-50/30 rounded-xl p-4 mb-3" data-testid="preset-form">
              <div className="flex items-center justify-between mb-3">
                <p className="text-sm font-semibold text-slate-900">{presetForm.id ? 'Edit preset' : 'New preset'}</p>
                <button type="button" onClick={() => setPresetForm(null)} data-testid="preset-form-close"
                  className="p-1 text-slate-400 hover:text-slate-600 rounded"><X className="h-4 w-4" /></button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <Field label="Name">
                  <input value={presetForm.name} data-testid="preset-name"
                    onChange={e => setPresetForm({ ...presetForm, name: e.target.value })}
                    className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" placeholder="e.g. Summer Campaign" />
                </Field>
                <Field label="Description">
                  <input value={presetForm.description || ''} data-testid="preset-description"
                    onChange={e => setPresetForm({ ...presetForm, description: e.target.value })}
                    className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" placeholder="Optional" />
                </Field>
                <Field label="Primary colour">
                  <ColorInput value={presetForm.primary_color} onChange={(v: string) => setPresetForm({ ...presetForm, primary_color: v })} />
                </Field>
                <Field label="Cert accent colour">
                  <ColorInput value={presetForm.cert_accent_color} onChange={(v: string) => setPresetForm({ ...presetForm, cert_accent_color: v })} />
                </Field>
                <Field label="Signature suggestion" help="Used only if your signature is still empty">
                  <input value={presetForm.cert_signature_text_suggestion || ''} data-testid="preset-signature"
                    onChange={e => setPresetForm({ ...presetForm, cert_signature_text_suggestion: e.target.value })}
                    className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" placeholder="e.g. Programme Director" />
                </Field>
                <Field label="Footer suggestion" help="Used only if your cert footer is still empty">
                  <input value={presetForm.cert_footer_text_suggestion || ''} data-testid="preset-footer"
                    onChange={e => setPresetForm({ ...presetForm, cert_footer_text_suggestion: e.target.value })}
                    className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" placeholder="e.g. International Fitness Professionals Institute" />
                </Field>
              </div>
              <div className="flex justify-end mt-3">
                <button type="button" onClick={savePreset} disabled={savingPreset} data-testid="preset-save-btn"
                  className="text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 px-4 py-2 rounded-lg inline-flex items-center gap-1.5">
                  <Check className="h-3.5 w-3.5" /> {presetForm.id ? 'Save changes' : 'Create preset'}
                </button>
              </div>
            </div>
          )}
                      ...
                      {t.custom && (
                        <>
                          <button type="button" onClick={() => setPresetForm({ ...t })}
                            data-testid={`theme-edit-${t.slug}`}
                            className="text-[11px] font-medium text-slate-500 hover:text-slate-700 hover:bg-slate-100 px-2 py-1 rounded-md inline-flex items-center gap-1">
                            <Pencil className="h-3 w-3" /> Edit
                          </button>
                          <button type="button" onClick={() => deletePreset(t)}
                            data-testid={`theme-delete-${t.slug}`}
                            className="text-[11px] font-medium text-red-500 hover:text-red-700 hover:bg-red-50 px-2 py-1 rounded-md inline-flex items-center gap-1">
